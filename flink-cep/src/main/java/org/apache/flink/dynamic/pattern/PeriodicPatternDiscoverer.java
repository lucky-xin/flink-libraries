...
/**
 * Implementation of the {@link PatternDiscoverer} that periodically discovers the pattern
 * processor updates.
 *
 * @param <T> Base type of the elements appearing in the pattern.
 */
public abstract class PeriodicPatternDiscoverer<T>
        implements PatternDiscoverer<T> {
    ...
    /**
     * Creates a new {@link PatternDiscoverer} using the given initial {@link
     * DynamicPattern} and the time interval how often to check the pattern processor updates.
     *
     * @param intervalMillis Time interval in milliseconds how often to check updates.
     */
    protected PeriodicPatternDiscoverer(final Long intervalMillis) {
     ...
     *
     * @return The list of {@link DynamicPattern}.
     */
    public abstract List<DynamicPattern<T>> getLatestPatternProcessors() throws Exception;
                ...
                new TimerTask() {
                    ...
                    public void run() {
                        if (arePatternProcessorsUpdated()) {
                            List<DynamicPattern<T>> patternProcessors = null;
                            try {

...
/**
 * Default implementation of the {@link DynamicPattern} that is configurable for {@link Pattern},
 ...
 */
@PublicEvolving
public class DefaultDynamicPattern<T> implements DynamicPattern<T> {
    ...
    @Override
    public <F extends T> Pattern<T, F> toPattern(ClassLoader classLoader) {
        try {
            return CepConverter.toPattern(patternStr, classLoader);
        } catch (Exception e) {

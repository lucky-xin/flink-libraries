...
import org.apache.flink.dynamic.event.UpdatePatternProcessorEvent;
...
import static org.apache.flink.dynamic.operator.CepOperator.PATTERN_MATCHED_TIMES_METRIC_NAME;
import static org.apache.flink.dynamic.operator.CepOperator.PATTERN_MATCHING_AVG_TIME_METRIC_NAME;
...
public class DynamicCepOperator<IN, KEY, OUT> extends AbstractStreamOperator<OUT>
        ...
        if (patternProcessors != null) {
            for (Map.Entry<PatternProcessor<IN>, NFA<IN>> entry : patternProcessors.entrySet()) {
                ...
                if (isProcessingTime) {
                    ...
                    final PatternProcessor<IN> processor = entry.getKey();
                    final PatternProcessFunction<IN, ?> patternProcessFunction = processor.getPatternProcessFunction();
                    // there can be no out of order elements in processing time
                    final NFAState nfaState = getNFAState(nfa);
                    long timestamp = processingTimeService.getCurrentProcessingTime();
                    advanceTime(processor, nfa, nfaState, timestamp, patternProcessFunction);
                    processEvent(
                            nfa,
                            nfaState,
                            element.getValue(),
                    ...
                    if (timestamp > timerServices.get(nfa).currentWatermark()) {
                        ...
                        numLateRecordsDroppedMetricMap.get(
                                Tuple2.of(entry.getKey().getId(), entry.getKey().getVersion())
                        ).inc();
                    }
                }
            }
    ...
    private void advanceTime(
        ...
        try (SharedBufferAccessor<IN> sharedBufferAccessor = partialMatches.get(nfa).getAccessor()) {
            AfterMatchSkipStrategy strategy = patternProcessor.getPattern(this.getUserCodeClassloader())
                    .getAfterMatchSkipStrategy();
            Tuple2<Collection<Map<String, List<IN>>>, Collection<Tuple2<Map<String, List<IN>>, Long>>>
                    pendingMatchesAndTimeout = nfa.advanceTime(sharedBufferAccessor, nfaState, timestamp, strategy);
    ...
    private void handleUpdatePatternProcessorEvent(UpdatePatternProcessorEvent<IN> event) throws Exception {
        ...
        for (PatternProcessor<IN> processor : eventPatternProcessors) {
            final Tuple2<String, Integer> patternProcessorIdentifier =
                    Tuple2.of(processor.getId(), processor.getVersion());
            final PatternProcessFunction<IN, ?> patternProcessFunction = processor.getPatternProcessFunction();
            FunctionUtils.setFunctionRuntimeContext(patternProcessFunction, cepRuntimeContext);
            FunctionUtils.openFunction(patternProcessFunction, DefaultOpenContext.INSTANCE);

            final NFACompiler.NFAFactory<IN> nfaFactory =
                    NFACompiler.compileFactory(
                            processor.getPattern(this.getUserCodeClassloader()),
            ...
            final InternalTimerService<VoidNamespace> timerService =
                    getInternalTimerService(
                            String.format(
                                    ...
                                    processor.getId(), processor.getVersion()),
                            VoidNamespaceSerializer.INSTANCE,
                            this);
            final TimerService cepTimerService = new TimerServiceImpl(nfa);
            patternProcessors.put(processor, nfa);

            LOG.info("Use patternProcessors version: {} with {}", processor.getVersion(), processor.getPattern(this.getUserCodeClassloader()));
    ...
    private void clearOutdatedComputationState(List<PatternProcessor<IN>> eventPatternProcessors) {
        if (computationStates != null && !computationStates.isEmpty()) {
            clearState(
                    ...
                    patternProcessors,
                    p -> computationStates.get(patternProcessors.get(p)).clear());
        }
    }
    ...
    private void clearOutdatedElementQueueState(List<PatternProcessor<IN>> eventPatternProcessors) {
        if (elementQueueStates != null && !elementQueueStates.isEmpty()) {
            clearState(
                    eventPatternProcessors,
                    patternProcessors,
                    p -> elementQueueStates.get(patternProcessors.get(p)).clear());
        }
    }
    ...
    private void clearOutdatedSharedBufferState(List<PatternProcessor<IN>> eventPatternProcessors) {
        if (partialMatches != null && !partialMatches.isEmpty()) {
            clearState(
                    ...
                    patternProcessors,
                    p -> {
                        SharedBuffer<IN> buffer = partialMatches.get(patternProcessors.get(p));
                        SharedBufferAccessor<IN> accessor = buffer.getAccessor();
                        try {
                            accessor.close();
                        } catch (Exception e) {
                            throw new IllegalStateException(e);
    ...
    public class ContextFunctionImpl implements PatternProcessFunction.Context {
        ...
        public <X> void output(final OutputTag<X> outputTag, final X value) {
            final StreamRecord<X> streamRecord;
            if (isProcessingTime) {
                streamRecord = new StreamRecord<>(value);
            } else {
                streamRecord = new StreamRecord<>(value, timestamp());
            }

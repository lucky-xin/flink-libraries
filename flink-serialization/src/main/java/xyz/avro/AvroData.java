...
public class AvroData {
    ...
    private static final Map<Schema.Type, List<Class<?>>> SIMPLE_AVRO_SCHEMA_TYPES = new EnumMap<>(Schema.Type.class);
    ...
    private final boolean discardTypeDocDefault;
    private final boolean allowOptionalMapKey;

    public AvroData(int cacheSize) {
        this(new AvroDataConfig.Builder()
                .with(AbstractDataConfig.SCHEMAS_CACHE_SIZE_CONFIG, cacheSize)
                .build());
    }
            ...
            switch (schemaType) {
                ...
                case STRUCT: {
                    ...
                    if (isUnionSchema(schema)) {
                        ...
                        org.apache.avro.Schema underlyingAvroSchema = avroSchemaForUnderlyingTypeIfOptional(schema, avroSchema, scrubInvalidNames);
    ...
    public org.apache.avro.Schema fromConnectSchema(Schema schema,
        ...
        if (!baseSchema.getType().equals(org.apache.avro.Schema.Type.UNION)) {
            if (connectMetaData) {
                ...
                if (schema.defaultValue() != null && (discardTypeDocDefault || schema.parameters() == null
                        || !schema.parameters().containsKey(AVRO_FIELD_DEFAULT_FLAG_PROP))) {
                    baseSchema.addProp(CONNECT_DEFAULT_VALUE_PROP, defaultValueFromConnect(schema, schema.defaultValue()));
                }
    ...
    private JsonNode parametersFromConnect(Map<String, String> params) {
        ...
        for (Map.Entry<String, String> entry : params.entrySet()) {
            if (ARG_PROPERTIES_PROP.equals(entry.getKey())) {
                try {
                    result.set(entry.getKey(), MAPPER.readTree(entry.getValue()));
                } catch (IOException e) {
                    throw new IllegalStateException("parse json data failed");
                }
                continue;
            }
    ...
    private Schema toConnectSchema(org.apache.avro.Schema schema,
        ...
        if (connectMetaData && parameters != null) {
            ...
            for (Map.Entry<String, Object> field : ((Map<String, Object>) parameters).entrySet()) {
